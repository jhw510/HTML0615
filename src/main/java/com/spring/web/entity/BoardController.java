...
// 사용자의 URL 요청을 처리하겠다.
//@RestController   //무조건 리턴값이 JSON이다
//JSON으로만 리턴값을 받을 땐 RestController고 그 외 다양하게 값을 받아도 되면 CONTROLLER 쓴다
//@Controller  일때 리턴값이 html인데  @RequestBody를하면  리턴값을 JSON으로 받을수있다
@Controller
...
public class BoardController {
    ...
    public ResponseEntity<byte[]> fileServe() throws Exception {
        ...
        try {
            ...
            //MediaType.APPLICATION_OCTET_STREAM 은 보통 파일을 다운받거나 할 때 사용한다
            headers.setContentType(MediaType.APPLICATION_OCTET_STREAM);
            //UTF-8이 나은 이유는 파일을 다운로드 할 때 인코딩 포맷을 맞춰야 하기 때문에
            //UTF-8내부에서 캐릭터 셋 설정이 있는데
            //ISO-8859-1 을 넣어줘야 파일이 잘 다운로드 된다
